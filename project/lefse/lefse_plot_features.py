...
def read_data(file_data, file_feats, params):
    ...
    for f in feats_to_plot:
        if params['f'] == "diff" and not f[1]:
            continue
        if params['f'] == "one" and f[0][0] != params['feature_name']:
            continue
...
def plot_features():
    ...
    if params['archive'] == "zip":
        file = zipfile.ZipFile(params['output_file'], "w")
    for k, f in features.items():
        ...
        if params['archive'] == "zip":
            ...
            if params['f'] == 'one':
                plot(params['output_file'], k, f, params)
            else:
                plot(params['output_file'] + str(int(f['sig'])) + "_" + "-".join(k.split(".")) + "." + params['format'], k, f, params)
    if params['archive'] == "zip":
        file.close()
